...
interface MaintenanceQueueMessage {
...
function getMinImportance(env: Env): number {
  const value = Number((env as unknown as Record<string, unknown>).MEMORY_MIN_IMPORTANCE || 0.55);
...
export async function runMemoryMaintenance(
  ...
  if ((env as unknown as Record<string, unknown>).ENABLE_INCREMENTAL_MEMORY !== "true") return { processed: false };
