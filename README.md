...
| 分支 | 是什么 | 适合谁 |
|---|---|---|
| `main` | **v1 稳定版**。本 README 描述的就是它。功能齐、跑得稳，不再加新功能，只修必要的坑 | 已稳定使用旧版/具有一定的动手能力的 |
| [`memory-v2`](../../tree/memory-v2) | **v2 记忆系统，作者自己线上跑的版本**。六层记忆分层、三档写入（即时 / 每4小时 / 凌晨 dream）、v4 Prompt Assembler（缓存锚点 + 时间戳自动拆分，Claude prompt cache 命中率显著更高）、boot 包 + 召回去重三闸 | 最新入手本仓库/想省心不想自己折腾的 |
